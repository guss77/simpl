...
contains wrapped calls to csimplmodule.so (C simpl extensions) in conjunction
with convenient packing/unpacking routines w.r.t. C type data items.
Wrapped-C-SIMPL
"""
...
class Simpl:
	...
	def nameAttach(self, name, maxMemSize):
		return nameAttach(name, maxMemSize)
...
def nameAttach(name, maxMemSize):
	...
	ret = csimpl.nameAttach(name)
	return ret

def nameDetach():
	ret = csimpl.nameDetach()
	return ret

def surDetach(fd):
	ret = csimpl.surDetach(fd)
	return fd

def nameLocate(phn):
	ret = csimpl.nameLocate(phn)
	return ret
...
def send(receiverId):
	...
	inBuffer = "\000" * maxIn
	inCtr = 0
	ret = csimpl.send(receiverId, outBuffer, inBuffer)
	outBuffer = ""
	outCtr = 0
	return ret
	
def trigger(receiverId, proxy):
	ret = csimpl.trigger(receiverId, proxy)
	return ret
...
def receive():
	...
	inBuffer = ""
	inCtr = 0
	messageSize, senderId, inBuffer = csimpl.receive()
	return messageSize, senderId

def reply(senderId):
	global outBuffer
	global outCtr
	ret = csimpl.reply(senderId, outBuffer)
	outBuffer = ""
	outCtr = 0
	return ret

def whatsMyReceiveFd():
	ret = csimpl.whatsMyReceiveFd()
	return ret
	
def whatsMyError():
	ret = csimpl.whatsMyError()
	return ret
	
def returnProxy(value):
	ret = csimpl.returnProxy(value)
	return ret
